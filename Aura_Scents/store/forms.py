...
class RegistrationForm(forms.ModelForm):
    ...
    referral_code = forms.CharField(
        ...
        help_text="Enter a referral code if you were referred by someone"
    )

    class Meta:
        model = User
        fields = ['username', 'email', 'password1', 'password2']  # Exclude referral_code from model fields
    ...
    def clean_referral_code(self):
        ...
        if referral_code:
            try:
                ...
                raise forms.ValidationError("The provided referral code is invalid.")
        return referral_code

    def save(self, commit=True):
        user = super().save(commit=False)
        user.set_password(self.cleaned_data['password1'])
        # Do NOT set user.referral_code here; let User.save handle it
        if commit:
            user.save()
        return user
